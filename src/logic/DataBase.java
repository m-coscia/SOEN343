...
public class DataBase {
    ...
    private DataBase() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // I THINK THE ISSUE IS BCZ THIS IS DONE BEFORE THE HOUSE LAYOUT IS SET SO THE
                // FINDROOM() ALWAYS IS NULL
    ...
    public void addAccount(Profile p) {
        ...
        try {
            ...
            if (p instanceof Parent) {
                ...
                // query = "INSERT INTO smart_home_simulator_db.profiles VALUES ('PARENT',
             ...
             * if( p instanceof Parent){
             * fw.write("PARENT," + p.getName() +"," + ((Parent) p).getUserName() + "," +
             * ((Parent) p).getPassword() + "," +((Parent) p).getLocation().getId()+ "\n" );
             * }else if (p instanceof Child){
             * fw.write("CHILD," + p.getName() +"," + ((Child) p).getUserName() + "," +
             * ((Child) p).getPassword() + "," +((Child) p).getLocation().getId()+ "\n" );
             * }else if (p instanceof Guest){
             * fw.write("GUEST," + p.getName() +"," + ((Guest) p).getUserName() + "," +
             * ((Guest) p).getPassword() + "," +((Guest) p).getLocation().getId()+ "\n" );
             * }else if(p instanceof Stranger){
             * fw.write("STRANGER," + p.getName() + "," +((Stranger)
             * p).getLocation().getId()+ "\n" );
             * }
             * fw.close();
             */
    ...
    public boolean profileExists(Profile userProfile) {
        ...
        return false;
        // System.out.println("you are in the profileExists() method: "
        // +userProfile.getName());
        // String sql = "SELECT FROM FullProfiles WHERE Type = ? AND Name =
        // \'"+userProfile.getName() +" ";
        //
        // try{
    ...
    public Profile findProfile(String name) {
        ...
        //
        // try (Connection con =
        // DriverManager.getConnection("jdbc:mysql://localhost/mysql", "root", "123");
